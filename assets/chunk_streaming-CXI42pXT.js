...
class ChunkInfoOverlay {
  textDiv_;
  imageLayer_;
  constructor({ textDiv, imageLayer: imageLayer2 }) {
    this.textDiv_ = textDiv;
    this.imageLayer_ = imageLayer2;
  }
  update(idetik, _timestamp) {
    ...
    const lodCounters = Array.from({ length: chunkManagerSource.lodCount }, () => ({
      visible: 0,
      rendered: 0,
      prefetched: 0
    }));
...
const overlaySelector = document.querySelector("#chunk-info");
...
const z = { translate: 0, scale: 1.24, shape: 448 };
const min = z.translate;
const max = z.translate + z.scale * z.shape - z.scale;
const zRange = { min, max };
const gui = new GUI({ width: 500 });
gui.add(controls.sliceCoords, "z", zRange.min, zRange.max, z.scale).name("Z-point");
gui.add(controls, "showWireframes").name("Show tile wireframes").onChange((show) => imageLayer.debugMode = show);
gui.add(controls, "showChunkInfoOverlay").name("Show chunk information overlay").onChange((show) => {
  overlaySelector.style.display = show ? "block" : "none";
});
//# sourceMappingURL=chunk_streaming-CXI42pXT.js.map
