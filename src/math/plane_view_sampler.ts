...
const MIN_APPROACH_COSINE = 1e-6;
...
const SAMPLE_COUNT = SAMPLE_NDC.length ** 2;

const unmeasuredFootprints = (): number[] =>
  new Array(SAMPLE_COUNT).fill(Infinity);
...
export class PlaneViewSampler {
  ...
  private readonly scratch_ = {
    ...
    far: vec3.create(),
    toFar: vec3.create(),
    cornerUV: new Float32Array(8),
    planeToClip: mat3.create(),
    adjugate: mat3.create(),
    planePoint: vec3.create(),
    boxMin: vec2.create(),
    boxMax: vec2.create(),
  };
  ...
  public view(
    ...
    let slot = 0;
    for (const [cornerNdcX, cornerNdcY] of CORNER_NDC) {
      const corner = this.intersect(cornerNdcX, cornerNdcY);
      ...
      cornerUV[slot++] = corner.u;
      cornerUV[slot++] = corner.v;
    }
  ...
  private intersect(ndcX: number, ndcY: number): RayHit {
    ...
    const approachCosine = approachRate / vec3.length(toFar);
    if (Math.abs(approachCosine) < MIN_APPROACH_COSINE) {
  ...
  /**
   * Mapping of homogeneous plane coordinates (u, v, 1) to clip space (x, y, w).
   * The bottom row (z) drops out because depth doesn't affect where a point lands on screen.
   */
  ...
  /*
   * The "footprint" measures units of the *plane* (geometric mean of both dimensions) covered by a
   * single pixel on the *screen*. This function samples that value at regular intervals across the
   * viewport.
   *
   * This function measures these footprints on a grid of screen-space locations described by
   * SAMPLE_NDC.
   *
   * smaller footprint -> finer LOD required
   * larger footprint -> coarser LOD sufficient
   * unmeasurable footprint (Infinity) -> coarsest LOD
   */
  private measureFootprints(
    ...
    // the adjugate (mat3.adjoint, similar to an inverse) maps a screen position back
    // onto the plane, with the determinant factored out:
    //
    //     adjugate * (ndcX, ndcY, 1) = (determinant / clipW) * (u, v, 1)
    //
    // we only care about clipW (from the third component) here, but the rest could give
    // the intersection location
    const { adjugate, ndc, planePoint } = this.scratch_;
    mat3.adjoint(adjugate, planeToClip);

    const footprints = new Array<number>(SAMPLE_COUNT);

    let slot = 0;
    for (const ndcY of SAMPLE_NDC) {
      for (const ndcX of SAMPLE_NDC) {
        vec3.transformMat3(planePoint, vec3.set(ndc, ndcX, ndcY, 1), adjugate);
        const clipW = determinant / planePoint[2];

        footprints[slot++] =
          clipW > MIN_CLIP_W && pixelArea > 0
            ? Math.sqrt((clipW * clipW * clipW) / pixelArea)
            : Infinity;
      }
    }
