...
test("submit two concurrent tasks", async () => {
...
test("submit one pending task", async () => {
  const executor = new TaskExecutor(1);
  let blocked = true;
  const promise0 = executor.submit(async () => {
    while (blocked) {
...
test("cancel one pending task", async () => {
  const executor = new TaskExecutor(1);
  let blocked = true;
  const promise0 = executor.submit(async () => {
    while (blocked) {
...
test("submit task that throws", async () => {
  const executor = new TaskExecutor(1);
  let blocked = true;
  const promise = executor.submit(async () => {
    while (blocked) {
