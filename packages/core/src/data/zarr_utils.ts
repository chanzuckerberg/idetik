...
type ChunkData = {
  data: ArrayBuffer | SharedArrayBuffer | Uint8Array | Int8Array | Uint16Array | 
  ...
  public async getChunk(
    ...
    if (!this.chunkCache_.has(key)) {
      ...
      try {
        ...
        const chunkPromise = this.array_.getChunk(chunkIndices, options) as Promise<ChunkData>;
      ...
      get(target, prop) {
        if (prop === 'getChunk') {
          ...
          return (chunkIndices: number[], chunkOptions: ChunkOptions = {}) => {
            return getChunkMethod(chunkIndices, { ...options, ...chunkOptions });
          };
        }
