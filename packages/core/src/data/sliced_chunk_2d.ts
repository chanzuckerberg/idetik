...
export class SlicedChunk2D {
  ...
  private constructor(props: SlicedChunk2DProps) {
  ...
  public updateZSlice(zCoord?: number): ChunkData {
    const channelSize = this.rowStride * this.shape.y;
    for (let c = 0; c < this.chunks.length; c++) {
      const chunk = this.chunks[c];
      const chunkData = slicePlane(chunk, zCoord);
      this.data.set(chunkData, c * channelSize);
    }
    return this.data;
  }
  ...
  static fromChunks(
    ...
    const TypedArray = refChunk.data!.constructor as new (
      size: number
    ) => ChunkData;
    const sortedChunks = chunks
      .slice()
      .sort((a, b) => a.chunkIndex.c - b.chunkIndex.c);
    const channelSize = refChunk.rowStride * refChunk.shape.y;
    const data = new TypedArray(sortedChunks.length * channelSize);

    const sliced = new SlicedChunk2D({
      chunks: sortedChunks,
      data,
      lod: refChunk.lod,
      shape: {
    ...
    sliced.updateZSlice(zCoord);
    return sliced;
  }
}

function slicePlane(chunk: Chunk, zCoord?: number): ChunkData {
  if (!chunk.data) {
    throw new Error("Chunk data is not loaded");
  }
