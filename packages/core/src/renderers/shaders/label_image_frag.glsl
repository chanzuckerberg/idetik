...
uniform highp usampler2D ColorLutSampler;
...
void main() {
    ...
    uint mapLength = uint(textureSize(ColorLutSampler, 0).x);
    for (uint i = 0u; i < mapLength; ++i) {
        uint key = texelFetch(ColorLutSampler, ivec2(i, 0), 0).r;
        if (texel == key) {
            uint value = texelFetch(ColorLutSampler, ivec2(i, 1), 0).r;
