...
const url =
  "http://localhost:8000/allencell-dynamic-nuc-morph/baseline_colonies_fov_timelapse_dataset/20200323_09_small/raw.ome.zarr";

// Source is 5D, so provide indices at 3 dimensions to project to 2D.
const source = new OmeZarrImageSource(url);

// values copied from source
const imageDataRange = { min: 0, max: 50000 };
const z = { translate: 0.0, scale: 0.7579, shape: 42 };
const zMin = z.translate;
const zMax = z.translate + z.scale * z.shape - z.scale;
const zRange = { min: zMin, max: zMax };

const t = { translate: 0.0, scale: 5.003550305, shape: 570 };
