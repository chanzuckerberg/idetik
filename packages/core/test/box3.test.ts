...
test("isEmpty detects inverted bounds", () => {
  const invX = new Box3({
    min: vec3.fromValues(1, 0, 0),
    max: vec3.fromValues(0, 1, 1),
  });
  const invY = new Box3({
    min: vec3.fromValues(0, 1, 0),
    max: vec3.fromValues(1, 0, 1),
  });
  const invZ = new Box3({
    min: vec3.fromValues(0, 0, 1),
...
test("intersects: touching edges do not count as intersecting", () => {
  ...
  const b = new Box3({
    min: vec3.fromValues(2, 0, 0),
    max: vec3.fromValues(4, 2, 2),
  });
  const c = new Box3({
    min: vec3.fromValues(2, 2, 0),
    max: vec3.fromValues(4, 4, 2),
  });
  const d = new Box3({
    min: vec3.fromValues(2, 2, 2),
    max: vec3.fromValues(4, 4, 4),
  });
  expect(Box3.intersects(a, b)).toBe(false);
...
test("apply transform with translation shifts box by offset", () => {
  const b = new Box3({
    min: vec3.fromValues(-1, 2, 3),
    max: vec3.fromValues(4, 5, 6),
  });
