...
export function useOmeZarrViewer({
    ...
    }, [source, sourceUrl, region, seriesDimensionName, zoomToFit, onLayerCreated, onFirstSliceLoaded]);
    ...
    useEffect(() => {
        const fetchZRange = async () => {
            ...
            const zIdx = attrs.dimensionNames.findIndex(d => d === seriesDimensionName);
            const min = 0;
